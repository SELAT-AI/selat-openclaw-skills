...
| [`selat-purchasing`](skills/selat-purchasing/SKILL.md) | Purchasing engine for agents on a **MetaMask Agent Wallet** — discover paid APIs by intent, buy on rails the native x402 payer can't sign, fund a Gateway budget gaslessly, report spend. Unlike the wrappers above it needs **no `selat` CLI or SELAT plugin**: it runs on the `mm` CLI + [selat-metamask-skills](https://github.com/SELAT-AI/selat-metamask-skills) |
